"""
This file contains functions to calibrate a single camera and a pair of cameras.
As well as functions to rectify a pair of images.
"""
    ...
    for img in images:
        ...
        if ret == True:
            corners = cv.cornerSubPix(gray, corners, (11, 11), (-1, -1),
                                      corner_detection_param)  # improves corner accuracy
...
def stereo_calibrate(mtx1, dist1, mtx2, dist2, images1_, images2_, camera_name1, camera_name2):
    """
    Stereo calibrate a pair of cameras
    Args:
        mtx1:
        dist1:
        mtx2:
...
def stereo_rectify(mtx1, dist1, mtx2, dist2, R, T, width, height):
    """
    Stereo rectify a pair of cameras
    Args:
        mtx1:
        dist1:
        mtx2:
...
def calibrate_3_cameras_to_file(filename='calibration_data.pkl'):
    ...
    ret_left, mtx_left, dist_left, rvecs_left, tvecs_left, corners_img_left, corners_3d_left = calibrate_camera(
        left_images, 'left')
    ret_middle, mtx_middle, dist_middle, rvecs_middle, tvecs_middle, corners_img_middle, corners_3d_middle = calibrate_camera(
        middle_images, 'middle')
    ret_right, mtx_right, dist_right, rvecs_right, tvecs_right, corners_img_right, corners_3d_right = calibrate_camera(
        right_images, 'right')

    # Stereo calibrate the 2 pairs of cameras
    ret_lm, CM1_lm, dist1_lm, CM2_lm, dist2_lm, R_lm, T_lm, E_lm, F_lm = stereo_calibrate(mtx_left, dist_left,
                                                                                          mtx_middle, dist_middle,
                                                                                          left_images, middle_images,
                                                                                          'left', 'middle')
    ret_mr, CM1_mr, dist1_mr, CM2_mr, dist2_mr, R_mr, T_mr, E_mr, F_mr = stereo_calibrate(mtx_middle, dist_middle,
                                                                                          mtx_right, dist_right,
                                                                                          middle_images, right_images,
                                                                                          'middle', 'right')
